...
PagoSchema.post("save", async function (doc) {
  if (doc.estadoPago === "completado") {
    console.log(
      `🔄 Payment ${doc._id} completed, updating boleta ${doc.boletaId} to 'pagada'`
    );
    ...
    if (boleta && boleta.estado !== "pagada") {
      ...
      console.log(
        `✅ Boleta ${doc.boletaId} updated to 'pagada' and user balance updated by ${doc.monto}`
      );
    } else if (boleta) {
      console.log(
        `ℹ️  Boleta ${doc.boletaId} already marked as 'pagada', skipping user balance update`
      );
    }
  }
});
...
PagoSchema.statics.updateBoletaStatus = async function (
  ...
  try {
    ...
    if (payment.estadoPago === "completado") {
      console.log(
        `💰 Processing completed payment ${paymentId} for boleta ${payment.boletaId}`
      );
      ...
      if (boleta && boleta.estado !== "pagada") {
        console.log(
          `🔄 Updating boleta ${payment.boletaId} from '${boleta.estado}' to 'pagada'`
        );
        ...
        if (user) {
          ...
          console.log(
            `✅ Boleta ${payment.boletaId} updated to 'pagada' and user balance updated by ${payment.monto}`
          );
        }
      } else if (boleta) {
        console.log(
          `ℹ️  Boleta ${payment.boletaId} already marked as '${boleta.estado}'`
        );
      } else {
