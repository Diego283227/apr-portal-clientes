...
const PaymentInterface: React.FC<PaymentInterfaceProps> = ({
  ...
  const [selectedBoletasForPayment, setSelectedBoletasForPayment] = useState<
    string[]
  ...
  const handleBoletaToggle = (boletaId: string, checked: boolean) => {
    if (checked) {
      ...
      setSelectedBoletasForPayment((prev) =>
  ...
  const getBoletasForPayment = () => {
    return selectedBoletas.filter((boleta) =>
      selectedBoletasForPayment.includes(boleta._id || boleta.id)
    );
  };

  const getTotalAmount = () => {
    return getBoletasForPayment().reduce(
              ...
              <CardContent className="space-y-4">
                ...
                {selectedBoletasForPayment.length === 0 && (
                  <Alert>
                    ...
                    <AlertDescription>
                      Debes seleccionar al menos una boleta para continuar con
                      el pago.
                    </AlertDescription>
                  </Alert>
                )}
