...
// Types
import type { User, Boleta, DashboardStats } from "@/types";
...
function App() {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      const cleanHashPath = hashPath.startsWith("/")
        ? hashPath.slice(1)
        : hashPath;
      ...
      if (
        ...
        (hash === "reset-password" || hash === "admin-reset-password") &&
        token
      ) {
      ...
      if (!isPasswordResetRoute) {
        ...
        if (authResult && currentUser) {
          ...
          if (!hash || guestRoutes.includes(hash)) {
            ...
            if (
              currentUser.role === "socio" &&
              validSocioRoutes.includes(baseRoute)
            ) {
              setCurrentView(baseRoute as AppView);
            } else if (
              currentUser.role === "super_admin" &&
              validAdminRoutes.includes(baseRoute)
            ) {
              setCurrentView(baseRoute as AppView);
            } else {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hashString.includes("/")) {
        ...
        if (
          (viewPart === "socio-dashboard" || viewPart === "chatbot") &&
          params[0]
        ) {
    ...
    switch (currentView) {
      ...
      case "socio-pago": {
        ...
        const boletasForPayment = selectedBoletaId
          ? boletas.filter(
              (b) =>
                b.id === selectedBoletaId || selectedBoletaId.includes(b.id)
            )
          : boletas.filter(
        ...
        return (
          <ProtectedRoute requiredRole="super_admin">
            <PagosAdminView
              ...
              onExportData={(dateRange) =>
                console.log("Export pagos:", dateRange)
              }
            />
          </ProtectedRoute>
        );
